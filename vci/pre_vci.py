...
def extract_images(paths: dict[str, str], args: argparse.Namespace) -> tuple[list[str], str]:
	...
	for i, img_path in tqdm(enumerate(images), desc="Scaling and copying images", total=len(filenames)):
		mask_path = os.path.join(paths['mask_source'], filenames[i] + "_mask.png")

		img = Image.open(img_path)
		mask = Image.open(mask_path)

		# Multiply mask on image as to remove background
		if args.use_masks:
			img = ImageChops.multiply(img, mask)

		img = scale_image(img, s)

		img.save(os.path.join(paths['input'], filenames[i] + "." + data_type_suffix))
...
def extract_masks(filenames: list[str], data_type_suffix: str, paths: dict[str, str], args: argparse.Namespace) -> None:
	...
	for i, file in tqdm(enumerate(filenames), desc="Scaling and copying masks", total=len(filenames)):
		...
		# Assert that there is a mask for every image
...
def extract_poses(filenames: list[str], data_type_suffix: str, paths: dict[str, str], args: argparse.Namespace) -> None:
	"""
	...
	for i, camera in tqdm(enumerate(calibration["cameras"]), desc="Reading camera calibration", total=len(calibration['cameras'])):
		...
		if not camera_name in filenames:
...
def main():
	...
	extract_poses(images, data_type_suffix, paths, args)
